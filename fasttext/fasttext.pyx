...
cdef class FastTextModelWrapper:
    ...
    def dict_get_word(self, i):
        ...
        return cpp_string.decode(self.encoding)
    ...
    def classifier_predict(self, text, k, label_prefix):
        ...
        text_bytes = bytes(text, self.encoding)
        labels = []
        raw_labels = self.fm.classifierPredict(text_bytes, k)
        for raw_label in raw_labels:
            label = raw_label.decode(self.encoding)
    ...
    def classifier_predict_prob(self, text, k, label_prefix):
        ...
        for result in raw_results:
            cpp_str = result[0]
            label = cpp_str.decode(self.encoding)
            label = label.replace(label_prefix, '')
            cpp_str = result[1]
            prob = float(cpp_str)
            labels.append(label)
...
def load_model(filename, label_prefix='', encoding='utf-8'):
    ...
    model = FastTextModelWrapper(encoding=encoding)
